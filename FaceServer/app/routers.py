...
async def add_face_handler(
    request: NewFaceRequest, face_model: FaceModelDependency, chromadb: ChromaDependency
) -> EmptyResponse:
...
async def delete_face_handler(
    user_id: str, chromadb: ChromaDependency
) -> EmptyResponse:
...
@router.get("/faces")
async def get_faces_handler(chromadb: ChromaDependency) -> list[User]:
