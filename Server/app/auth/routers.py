...
async def new_login(data: NewRobotLogin, db: DbDependency) -> Robot:
...
async def login(
    login_request: LoginRequest, redis_pool: RedisDependency, db: DbDependency
) -> LoginCode:
    token, request_id = await create_login_code_for_client(
        login_request.id, redis_pool, db, ClientType.ROBOT
    )

    return LoginCode(data=token, request_id=request_id)
...
async def login_code(
    ...
    session_id = await create_session(
        login_data.request_id, login_data.data, redis_pool, ClientType.ROBOT
    )
...
async def new_login(data: NewStoreLogin, db: DbDependency) -> Store:
    print("Store login code for", data.store_id)
    return await create_new_login_for_store(
        data.login, data.password, data.public_key, data.store_id, db
    )
...
async def login(
    ...
    token, request_id = await create_login_code_for_client(
        login_request.id, redis_pool, db, ClientType.STORE
    )
...
async def login_code(
    ...
    session_id = await create_session(
        login_data.request_id, login_data.data, redis_pool, ClientType.STORE
    )
