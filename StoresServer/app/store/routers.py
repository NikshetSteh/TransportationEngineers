...
async def get_items_handler(
    store_id: StoreAuthRequired, db: DbDependency
) -> Page[StoreItem]:
...
async def get_item_handler(
    store_id: StoreAuthRequired, item_id: str, db: DbDependency
) -> StoreItem:
...
async def add_item_handler(
    store_id: StoreAuthRequired, item: StoreItemCreation, db: DbDependency
) -> StoreItem:
...
async def update_item_handler(
    store_id: StoreAuthRequired, item: StoreItem, db: DbDependency
) -> StoreItem:
...
async def make_purchase_handler(
    store_id: StoreAuthRequired, purchase_data: PurchaseCreation, db: DbDependency
) -> Purchase:
...
async def mark_as_ready_handler(task_id: str, db: DbDependency) -> EmptyResponse:
...
async def get_tasks_handler(
    store_id: StoreAuthRequired, db: DbDependency, also_ready_tasks: bool = False
) -> Page[Task]:
