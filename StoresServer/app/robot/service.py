...
async def get_user_recommendations(
    store_id: str, user_id: str, db: sessionmaker[AsyncSession]
) -> list[StoreItem]:
    async with db() as session:
        ...
        last_purchases = (
            await session.execute(
                ...
                .join(
                    PurchaseItemModel,
                    PurchaseItemModel.store_item_id == StoreItemModel.id,
                )
                .join(PurchaseModel, PurchaseModel.id == PurchaseItemModel.purchase_id)
                .where(
                    PurchaseModel.user_id == user_id, PurchaseModel.store_id == store_id
                )
        ...
        user_popular_purchases = (
            await session.execute(
                select(StoreItemModel)
                .join(
                    PurchaseItemModel,
                    StoreItemModel.id == PurchaseItemModel.store_item_id,
                )
                .group_by(StoreItemModel.id)
                .where(
                    PurchaseItemModel.purchase_id.in_(
                        select(PurchaseModel.id).where(
        ...
        for item in recommendations:
            if item.id in items_ids:
                ...
                items_for_recommendations.append(
                    StoreItem(
                        id=str(item.id),
                        name=item.name,
...
async def get_store_list(ids: list[str], db: sessionmaker[AsyncSession]) -> list[Store]:
    async with db() as session:
        stores = (
            (await session.execute(select(StoreModel).where(StoreModel.id.in_(ids))))
            .scalars()
            .all()
        )
